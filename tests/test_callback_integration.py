...
async def test_pvp_callback_create_and_accept_persist_real_postgresql(
    ...
    try:
        ...
        # get_session is an async generator; the real setup happens below.
        pass
    ...
    try:
        ...
        async with get_session() as session:
            match = await get_active_match(session, creator.id)
        ...
        with (
            ...
            await dp.feed_update(bot, _update(f"pvp:accept:{match_id}", opponent_id))

        async with get_session() as session:
            match = await get_active_match(session, opponent.id)
            assert match is not None
        ...
        async with get_session() as session:
            await session.execute(
                delete(DbUser).where(DbUser.telegram_id.in_([creator_id, opponent_id]))
            )
